...
// --- Flachwitze ---
let jokesData = { jokes: [], intervalMinutes: 10, count: 3 };
let jokesEnabled = false;
let jokesLastRotation = 0;

async function loadJokesSettings() {
  const data = await fetchJSON('/api/settings/public');
  jokesEnabled = data.jokesEnabled !== false;
  const section = document.getElementById('jokesSection');
  if (section) section.style.display = jokesEnabled ? '' : 'none';
  return jokesEnabled;
}

// Fetch joke data + rotation config. Rotates the displayed jokes only when
// the configured interval has elapsed (or on first load / forced refresh).
async function loadJokes(forceRotate = false) {
  if (!jokesEnabled) return;
  try {
    jokesData = await fetchJSON('/api/jokes');
    maybeRotateJokes(forceRotate);
  } catch (err) {
    console.error('Fehler beim Laden der Witze:', err);
  }
}

function maybeRotateJokes(force = false) {
  const minutes = jokesData.intervalMinutes > 0 ? jokesData.intervalMinutes : 10;
  const elapsed = Date.now() - jokesLastRotation;
  if (force || jokesLastRotation === 0 || elapsed >= minutes * 60 * 1000) {
    rotateJokes();
    jokesLastRotation = Date.now();
  }
  // else: interval not elapsed — keep the currently displayed jokes
}

function pickRandomJokes(n) {
  const pool = [...jokesData.jokes];
  const picked = [];
...
function rotateJokes() {
  ...
  if (!jokesData.jokes || jokesData.jokes.length === 0) {
    list.innerHTML = '<div class="empty-state">Noch keine Witze hinterlegt</div>';
    return;
  }

  const count = jokesData.count > 0 ? jokesData.count : 3;
  const picked = pickRandomJokes(count);
  list.innerHTML = picked.map(j => `
    <div class="joke-item">${escapeHtml(j.text)}</div>
  `).join('');
}

function initJokes() {
  const btn = document.getElementById('jokesRefresh');
  if (btn) {
    btn.addEventListener('click', () => maybeRotateJokes(true));
  }
}
